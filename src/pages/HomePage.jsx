...
import {
  ...
  Lock, Clock, HardDrive, Radio, LayoutGrid,
} from "lucide-react";
...
function NetworkHealthBar({ health }) {
  ...
  const color =
    pct >= 70 ? "#22c55e" : pct >= 40 ? "#f59e0b" : pct > 0 ? "#ef4444" : "#3f3f46";
    ...
    <div className="page page-scroll px-6 py-6 flex flex-col gap-6">
      ...
      <section>
        ...
        <div className="grid grid-cols-2 gap-3">
          <StatCard icon={Users}    label="Active Peers"  value={peerCount}              sub="on this WiFi"        color="brand"   delay={0.05} />
          <StatCard icon={Radio}    label="Discovery"     value={stats.discoveryActive ? "Active" : "Idle"}  sub={broadcasting ? "Broadcasting" : "Not broadcasting"} color={stats.discoveryActive ? "success" : "surface"} delay={0.1} />
          <StatCard icon={Wifi}     label="Network"       value={stats.networkHealth === "unknown" ? "—" : stats.networkHealth} sub="connection quality" color={stats.networkHealth === "excellent" || stats.networkHealth === "good" ? "success" : "warning"} delay={0.15} />
          <StatCard icon={Lock}     label="Encryption"    value="AES-256"                sub="GCM · Active"        color="brand"   delay={0.2}  />
        </div>
        <div className="mt-3 px-4 py-3 rounded-xl border border-surface-800 bg-surface-900/40">
          <div className="flex justify-between items-center mb-2">
            ...
            <span className="text-xs font-semibold text-surface-400 capitalize">{stats.networkHealth}</span>
          </div>
          <NetworkHealthBar health={stats.networkHealth} />
        </div>
      </section>
      ...
      <section>
        ...
        <div className="grid grid-cols-2 gap-3">
          <StatCard icon={Clock}       label="Active Time"       value={timer}                   sub="since app opened"   color="surface" delay={0.1} />
