...
import {
  Shield, UserPlus, UserMinus, Camera, Send,
  Download, Radio, Users, MessageSquare, Zap, Trash2,
} from "lucide-react";
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5 flex-wrap">
          <span className="text-sm font-semibold text-surface-200 truncate">{event.actor}</span>
...
export default function AuditLog({ limit = 50 }) {
  ...
  const visible = auditEvents.slice(0, limit);
      ...
      <AnimatePresence initial={false}>
        {visible.length === 0 ? (
          <motion.div
            ...
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="flex flex-col items-center gap-2 py-10 text-surface-600"
          >
            <Shield size={24} />
